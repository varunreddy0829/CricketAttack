...
# and intents (stage 4, always last): pitch personality, innings phase, and
# one-shot gambit cards. Like every other stage it conserves the incoming
# weight sum (1000.0) and leaves the rare boundary types '3' and '5'
# untouched (they're near-static constants by design).

# Per-pitch multipliers. "vs_spin"/"vs_pace" apply only when the bowler's
# style matches; "all" applies regardless of who's bowling.
PITCH_EFFECTS = {
    "dusty": {
        "vs_spin": {"Out": 1.02, "0": 1.02},
        "vs_pace": {"Out": 0.99},
        "all": {"4": 0.96, "6": 0.96},
    },
    "green": {
        "vs_pace": {"Out": 1.02, "0": 1.01},
        "vs_spin": {"Out": 0.99},
        "all": {"4": 0.98},
    },
    "flat": {
    ...
    "slow": {
        "vs_spin": {"Out": 1.02},
        # 4/6 cut (but gently); 1/2 bumped more than 0 -- still lower-scoring
        # and boundary-shy, but the ball stays in play for a single/double
        # rather than dying as a dot, so it's not batting-hostile.
        "all": {"4": 0.93, "6": 0.90, "0": 1.02, "1": 1.10, "2": 1.10},
    },
}
...
GAMBIT_ATTACK_OUT_CUT = 0.40  # Out drops by 40% on top
...
GAMBIT_TRAP_BOUNDARY_CUT = 0.40   # '4' and '6' each drop by 40% on top
...
def apply_conditions(weights, ctx):
    """ctx keys (all optional): pitch, bowler_style, over_num,
